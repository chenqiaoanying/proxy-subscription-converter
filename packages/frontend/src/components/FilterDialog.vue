...
const subscriptionsById = computed(() =>
    subscriptions.value.reduce((map, subscription) => {
...
const supportedTypes = computed(() => {
    ...
    const selectedSubscriptions = selectAllSubscriptions
        ? subscriptions.value
        : filter.subscriptionIds?.map((id) => subscriptionsById.value.get(id)!) || []
    selectedSubscriptions
        .flatMap((subscription) => subscription.proxies)
        .forEach((proxy) => {
            if (!['selector', 'urltest', 'direct', 'dns', 'block'].includes(proxy.type))
...
function onConfirm() {
    ...
    const update = toUpdateFilter
        ? filterStore.updateFilter(toUpdateFilter.id, toSaveFilter)
        : filterStore.createFilter(toSaveFilter)
    update
...
<template>
    <el-dialog
        :title="toUpdateFilter ? '编辑过滤器' : '添加过滤器'"
        v-model="visible"
        @open="onOpen"
    >
        <el-form :model="filter" label-width="100px">
            <el-form-item label="名称">
                <el-input
                    v-model="filter.tag"
                    placeholder="请输入名称"
                    :disabled="toUpdateFilter != undefined"
                ></el-input>
            </el-form-item>
            ...
            <el-form-item label="订阅">
                <el-checkbox
                    v-show="filter.subscriptionIds?.length == 0"
                    v-model="selectAllSubscriptions"
                    label="所有订阅"
                    style="width: 100%"
                />
                <el-checkbox-group
                    v-show="!selectAllSubscriptions"
                    v-model="filter.subscriptionIds"
                >
                    <el-checkbox
