...
function onConfirm() {
    ...
    const update = toUpdateGenerator
        ? subscriptionGeneratorStore.updateGenerator(toUpdateGenerator.id, data)
        : subscriptionGeneratorStore.createGenerator(data)
    update
        ...
        <div class="drawer-content" style="display: flex; flex-direction: column; height: 100%">
            <el-form>
                ...
                <el-form-item v-show="subscriptionGenerator.type === 'url'">
                    <el-input
                        placeholder="请输入模板链接"
                        v-model="subscriptionGenerator.url"
                    ></el-input>
                </el-form-item>
            </el-form>
            <div
                v-show="subscriptionGenerator.type === 'json'"
